...
- **Effects Processing**: Per-voice overdrive distortion, plus a master-bus analog-style delay (fractional read, high feedback with a lowpass in the loop; wet-mix fader with Shift for delay time)
...
9. **Delay & groove:** Fader 2 sets the master delay wet mix (hold **Shift** and move the same fader for delay time, 10 ms–1 s, with tape-style pitch glides). Shuffle/swing comes from the 16 templates (Utility button 4); the old continuous swing fader was replaced by the delay.
