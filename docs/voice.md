...
- **`VoiceManager`**: Manages multiple voices with allocation, deallocation, master volume scaling, per-voice mix levels, unified block audio processing, and the master-bus glue compressor (`rpdsp::Compressor`, last DSP before the DAC).
