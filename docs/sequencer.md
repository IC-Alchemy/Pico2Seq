...
```
                              ...
                              │                           ▼                            │
                              │      onStepCallback()  (stage into the                │
                              │      clockEvents.steps SPSC ring,                     │
                              │      src/app/ClockService.cpp)                        │
                              │                           │                            │
                              │                           ▼  loop(): processClockEvents()
                              │      processSequencerStep() (src/app/StepPlayback.cpp) │
                              │                           │                            │
                              │        ┌──────────────────┴──────────────────┐         │
                              │        ▼                                     ▼         │
                              ...
                              │                       ▼                                │
                              │                  VoiceSystem                           │
                              │    (Gate Timers, Internal Note Lifecycle)              │
                              │                       │                                │
                              └───────────────────────┼────────────────────────────────┘
...
4. **Dual-Core Execution**:
   - **Core 0** runs `uClock` (timer ISR on core 0 — stock library). The ISR-stage-only `onStepCallback()` (in `src/app/ClockService.cpp`) queues each 16th note; `loop()` drains it (`ClockService::processClockEvents()` → `StepPlayback::processSequencerStep()`, both in `src/app/`), advancing all 4 sequencers, managing gate timers, and running the internal note-lifecycle bookkeeping in thread context.
...
### 4.2 Step Callback & Step Processing (`src/app/ClockService.cpp` / `src/app/StepPlayback.cpp`)

Since the PR #17 app-layer extraction, this logic no longer lives in `Pico2Seq.ino` (a 26-line shell that only calls `Application::begin()/update()` and `AudioEngine::begin()/renderNextBuffer()`).

`uClock` invokes `onStepCallback()` on Core 0 (timer ISR context) on every 16th note, but it only enqueues the step number into the 16-deep SPSC ring inside `ClockService.cpp`. `loop()` (via `Application::update()`) drains the queue through `ClockService::processClockEvents()` → `StepPlayback::processSequencerStep()` in thread context, which does the full step work:

```cpp
// src/app/ClockService.cpp — ISR context, stage only
...
// src/app/StepPlayback.cpp — thread context, drained by processClockEvents()
void processSequencerStep(uint32_t uClockCurrentStep)
{
    g_processedStepCount++; // freeze-forensics post-mortem counter
    ...
    // 1. Advance all 4 sequencers. Only the selected voice hears the sensor.
    for (uint8_t voice = 0; voice < VoiceSystem::MAX_VOICES; ++voice)
    {
        const int distance = voice == selectedVoice ? handDistance : -1;
        advanceSequencerStep(*AppState::sequencers[voice], uClockCurrentStep,
                             distance, uiState, &tempStates[voice]);
    }

    // 2. Apply encoder base values per voice (all 4 voices) before staging
    ...
    // 3. Update VoiceSystem: voices 0/1 keep gate timers & note lifecycle,
    //    voices 2/3 are audio synthesis only
...
- **Default Tempo**: 90 BPM (configured during `initializeClock()` in `src/app/ClockService.cpp`).
- **ISR handoff**: On Core 0, `onStepCallback()` queues step numbers into
  `clockEvents.steps` for `processClockEvents()`, while `onOutputPPQNCallback()`
  increments `ppqnTicksPending` — a plain `volatile uint32_t` with the original
  increment/decrement policy (`src/app/ClockService.cpp`).
- **PPQN drain**: After processing queued steps, `loop()` calls
  `processPendingGateTicks()`, which decrements the counter *before* processing
  each tick and advances MIDI note timing, sequencer note durations, and gate
  timers until the counter reads zero. Because the drain interleaves
  read/decrement/write on the volatile counter, an ISR increment landing between
  a decrement's load and store can be lost — this lost-increment window is
  pre-existing and still open.
- **Replacement pending**: `src/utils/PendingTickCounter.h` (lock-free atomic
  `post()` / `takeAll()` with a single `exchange(0)` snapshot, so no
  read/clear window exists) is host-tested under the `[ppqn]` tag and the
  standalone `pico2seq_clock_tests` target, but is **not wired into the clock
  path yet**.
...
| `ClockService` (free functions) | `src/app/ClockService.h/.cpp` | All uClock integration: `initializeClock()`, ISR stage-only callbacks (`clockEvents.steps` ring, `ppqnTicksPending`, `droppedSteps`), `processClockEvents()`, `processPendingGateTicks()`, transport callbacks |
| `StepPlayback` (free functions) | `src/app/StepPlayback.h/.cpp` | Core-0 thread-context step work: `processSequencerStep()`, recording entries `updateParametersForStep[Normalized]()`, `updateActiveVoiceState()`, `updateVoiceMIDI()` gate/note routing |
