...
4. **Dual-Core Execution**:
   - **Core 0** runs `uClock` (timer ISR on core 0 — stock library) and triggers `onStepCallback()` on every 16th note, advancing all 4 sequencers, managing gate timers, and sending MIDI note-on/off events.
...
In `Pico2Seq.ino`, `uClock` invokes `onStepCallback()` on Core 0 (timer ISR context) on every 16th note:
