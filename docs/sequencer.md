...
4. **Dual-Core Execution**:
   - **Core 1** runs `uClock`. Its ISR fires on this core and only queues the step number (`onStepCallback` → `StepTickQueue`); `loop1()` drains the queue and runs `processSequencerStep()`, which advances all 4 sequencers, manages gate timers, and sends MIDI note-on/off events.
...
### 4.2 Step Handoff: `onStepCallback` → `StepTickQueue` → `processSequencerStep`

In `Pico2Seq.ino`, `uClock` invokes `onStepCallback()` in ISR context on every 16th note.
The callback is ISR-minimal: it only records the step into `pendingUclockSteps` (a
`StepTickQueue` SPSC ring, `src/pico2seq-core/sequencer/StepTickQueue.h`). `loop1()` then
drains the queue in thread context and runs all of the real work in
`processSequencerStep()` — sequencer advance, encoder base values, `VoiceSystem` updates,
and MIDI all happen there, never in the ISR:
