...
void initOscillators()
{
    ...
    for (uint8_t i = 0; i < VoiceSystem::MAX_VOICES; i++)
    {
        ...
        VoiceParameters::seedTracks(*sequencers[i], VoicePresets::getPresetConfig(presetIndices[i]));
    }

    voicesReady.store(true, std::memory_order_release);
}

// Only repurposed timbre tracks are seeded; note and gate patterns remain intact.
static void seedRepurposedParamTracks(uint8_t voiceIndex, const VoiceConfig &config)
{
    ...
    VoiceParameters::seedTracks(*sequencers[voiceIndex], config);
}
