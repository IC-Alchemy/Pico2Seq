/*
 * I2C 4x/8x Button module — Alchemy Modular UI protocol v2 tile
 * MCU: PY32F030F28U6TR — same PCB and MCU as the slider tile
 *
 * Wiring (per schematic):
 *   PA0..PA3  - momentary buttons 0..3 (active-low, internal pull-ups)
 *   PA4..PA7  - buttons 4..7 on the 8x PCB (NUM_BUTTONS 8)
 *   PB5       - address strap (1k to GND / 1k to 3V3 / floating)
 *   PB6       - I2C1 SCL (AF6)
 *   PB7       - I2C1 SDA (AF6)
 * SCL/SDA need external pull-ups to 3V3 (4.7k typical).
 *
 * Both variants report the same identity — TYPE_ID 0x02, DATA_LEN 3 (spec
 * §7.2). The three button bytes are full 8-bit bitmaps, so the 8x PCB just
 * uses bits 4..7 of the same bytes; a 4x-aware hub parses the frame
 * unchanged, and no protocol version moves.
 *
 * Implements the v2 register protocol, TYPE_ID 0x02 (spec §7.2):
 * identity block 0x00-0x19, STATUS/DATA/SUM from 0x20 with DATA_LEN 3
 * (button levels, sticky pressed, sticky released — byte-identical to
 * slider-tile DATA bytes 8..10, so one hub parser serves both), config
 * 0x40-0x42, LED stubs 0x50-0x53, SOFT_CMD 0x7F.
 *
 * Address: 0x0B + strap offset, the Button 4x registry block from spec §4.3.
 * Strap offsets 0/1/2 therefore answer at 0x0B/0x0C/0x0D. Each button tile
 * on one bank must use a different strap state; slider tiles occupy their
 * separate 0x08-0x0A block and cannot collide with this module.
 *
 * This is the slider tile's button half as its own tile: no ADC
 * and no timers at all — one GPIO read per button per 4 ms sweep, debounced
 * on-tile, edges latched into sticky bits that survive any poll rate.
 *
 * ---------------------------------------------------------------------
...
#define PIN_STRAP PB5 // address strap, NOT ADC-capable — tri-state digital probe only

// --- Panel shape -----------------------------------------------------------
// The one knob that differs between the two button PCBs sharing this MCU:
// set NUM_BUTTONS to 4 (PA0..PA3) or 8 (PA0..PA7). Everything below —
// debounce arrays, sticky bitmaps, sweep loops — is sized from this macro.
#define NUM_BUTTONS 8
#if (NUM_BUTTONS != 4) && (NUM_BUTTONS != 8)
#error "NUM_BUTTONS must be 4 or 8"
#endif
#if NUM_BUTTONS == 8
const uint8_t buttonPins[NUM_BUTTONS] = { PA0, PA1, PA2, PA3, PA4, PA5, PA6, PA7 };
#else
const uint8_t buttonPins[NUM_BUTTONS] = { PA0, PA1, PA2, PA3 };
#endif
...
const uint8_t TYPE_BUTTON         = 0x02;
const uint8_t ADDR_BASE           = 0x0B;  // 0x0B, 0x0C, 0x0D
...
// Button 4x DATA layout, §7.2 — byte-identical to SliderTile DATA bytes
// 8..10 so one hub parser serves both tile types. Each byte is a full
// 8-bit bitmap: on the 8x PCB, buttons 4..7 are bits 4..7 of these same
// three bytes, so DATA_LEN and every offset stay untouched.
namespace ButtonTile {
const uint8_t DATA_LEN      = 3;
const uint8_t D_BTN_LEVEL    = 0;  // bit n = 1 while button n is held
const uint8_t D_BTN_PRESSED  = 1;  // sticky pressed-since-last-read, clear on read
const uint8_t D_BTN_RELEASED = 2;  // sticky released-since-last-read, clear on read
}
...
// but have no effect on a pure button tile: there are no faders to filter.
// CFG_DEBOUNCE defaults to 4 ms: Tier B poll is 4 ms, so 8 ms would add
// needless latency.
#define CFG_DEBOUNCE_DEFAULT_MS 4
#define SWEEP_INTERVAL_MS 4

// HW_REV is a compile-time constant for this board revision. Decided over a
// strap/fuse read: no provisioning step exists and revs are rare.
#define HW_REV 0x01
...
// and publishes it with a single index flip — no noInterrupts() anywhere in
// the sample path. The ISR latches which buffer to serve on the ADDR-match
// event and keeps serving that buffer for the whole transaction, so a sweep
// landing mid-read can never tear a frame.
#define FRAME_LEN 5  // 1 STATUS + DATA_LEN + 1 SUM
...
// frame (observed as sporadic SUM failures under soak). Skipping a publish
// costs one sweep of staleness; tearing costs integrity.
...
// saw, and a lost press is worse than a repeated one.
static void noteCursorPassed(uint8_t deliveredEnd)
{
  if (deliveredEnd > Proto::REG_DATA + Proto::ButtonTile::D_BTN_PRESSED) {
    stickyConsumed |= STICKY_PRESSED_BIT;
  }
  if (deliveredEnd > Proto::REG_DATA + Proto::ButtonTile::D_BTN_RELEASED) {
...
// SEQ advances only when the DATA block changed; NOT_READY stays set
// until the first sweep completes. Frozen SEQ + toggling HEARTBEAT = idle,
// both frozen = wedged.
...
static void publishFrame(void)
{
  ...
  f[0] = status;
  f[1 + Proto::ButtonTile::D_BTN_LEVEL]    = stableButtons;
  f[1 + Proto::ButtonTile::D_BTN_PRESSED]  = stickyPressed;
  f[1 + Proto::ButtonTile::D_BTN_RELEASED] = stickyReleased;

  // SEQ advances only when DATA actually changed (compare vs published).
  volatile const uint8_t* cur = frameBuf[activeFrame];
  bool changed = false;
  for (uint8_t i = 1; i <= Proto::ButtonTile::DATA_LEN; i++) {
  ...
  f[1 + Proto::ButtonTile::DATA_LEN] = sum;
...
void setup()
{
  ...
  regMap[Proto::REG_TYPE_ID]   = Proto::TYPE_BUTTON;
  ...
  regMap[Proto::REG_CAPS]      = (uint8_t)(Proto::CAP_DIGITAL_IN);
  ...
  regMap[Proto::REG_DATA_LEN] = Proto::ButtonTile::DATA_LEN;

  // Config page defaults: 4 ms debounce. CFG_RATE/CFG_FILTER accepted but
  // functionless on a button-only tile.
  regMap[Proto::REG_CFG_RATE]     = 0;
  regMap[Proto::REG_CFG_FILTER]   = Proto::cfgFilter(0, 0);
