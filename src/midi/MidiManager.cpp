...
#include "../pico2seq-core/scales/scales.h" // ADD THIS LINE
...
void allNotesOff()
{
    ...
    {
        seq1.handleNoteOff(&voiceSystem.getVoiceState(0));
    }
    if (seq2.isNotePlaying())
    {
        seq2.handleNoteOff(&voiceSystem.getVoiceState(1));
    }
...
void MidiNoteManager::updateParameterCC(uint8_t voiceId, ParamId paramId, float value)
{
    // Validate inputs
    if (voiceId > 1)
        return; // Only support Voice 1 (0) and Voice 2 (1)
...
bool MidiNoteManager::shouldTransmitCC(uint8_t voiceId, ParamId paramId, float value)
{
    ...
    if (currentTime - state.lastTransmissionTime < 10)
    { // 10ms minimum interval
        return false;
    }
