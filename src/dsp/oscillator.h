...
public:
  ...
  /** Sample-rate version accessor. Increments whenever Init() is called
   * (i.e. when sample-rate changes/reinitializes).
   */
  static inline uint32_t GetSampleRateVersion() {
    return s_srVersion.load(std::memory_order_seq_cst);
  }
...
private:
  ...
  // Global sample-rate version counter (bumps when Init() is called)
  static std::atomic<uint32_t> s_srVersion;
};
} // namespace daisysp
#endif
#endif
