...
AS5600Sensor::AS5600Sensor()
    : lastReadTime(0), rawAngle(0), lastRawAngle(0), sensorConnected(false), cumulativePosition(0), lastPosition(0), angularSpeed(0.0f), lastSpeedTime(0)
{
}
...
void AS5600Sensor::updateAngularSpeed(unsigned long currentTime)
{
    ...
    if (deltaTime < 8)
        return; // Too small time delta for accurate measurement
...
{
    if (absSpeed <= MIN_VELOCITY_SPEED)
        return 0.0f;
...
void AS5600Sensor::validateSmoothScaling() const
{
    ...
    for (int i = 0; i < numTests; i++)
    {
        ...
        if (testSpeeds[i] < 500.0f)
            Serial.println("Low-Speed Enhanced");
        else if (testSpeeds[i] > 1500.0f)
            Serial.println("High-Speed Enhanced");
        else
            Serial.println("Mid-Range Standard");
    }
