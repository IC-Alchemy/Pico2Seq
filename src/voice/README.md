...
| `Voice.cpp` | Shared pitch, gate/trigger, mixing and cross-core delivery (drone build: no per-voice filter or envelope) |
...
These patches connect existing rpDSP functions and render continuously as
drones — no per-voice filter, envelope or high-pass. They add no DSP
algorithms or per-voice storage.
The three controls below occupy the Filter, Attack and Decay lanes, respectively.

| Preset | Sound and rpDSP source | Timbre controls |
| --- | --- | --- |
| VelvetKeys | Soft electric keys: two `osc_fbfm` operators at a 2:1 ratio, low feedback, rounded tone | Index / Ratio / Feedback |
| CopperBass | Rounded, harmonically rich bass: `osc_dsf` at harmonic spacing plus a sine sub from `osc_pdmorph` | Bright / Spacing / Sub |
| ReedPipe | Held reed tone: `osc_formant` bursts blended with a sine fundamental | Formant / Bloom / Body |
| SilkPad | Slow orchestral pad: two gently detuned `osc_pdmorph` voices, free-running phase | Silk / Detune / Blend |
...
10 cents each way. Blend balances those two oscillators. Voices drone through
gate-off, so gate length schedules the next trigger rather than ending the
note; bell and pluck patches reset on each gate rise.
...
default output at C2/C4/C6, gate/retrigger handling, preset seeding and editor selection.
...
| Filter | First timbre control | Unbound (`--`) on the nine oscillator presets |
| Attack | Second timbre control | Unbound (`--`) on the nine oscillator presets |
| Decay | Third timbre control | Unbound (`--`) on the nine oscillator presets |
...
gate-off and retrigger handling: they drone through gate-off, and each gate
rise (or retrigger) resets the patch unless it was configured free-running.
The shared pitch path includes harmony, detune,
...
with another recipe, or bind other meaningful DSP members in the layout. Do not
expose controls that the patch ignores.
...
sources clears their state, so this is not a crossfade: the cleared engine
starts over from silence at the next gate-low span.
...
recipe control influence, pitch/glide, gate/retrigger and staged source changes.
